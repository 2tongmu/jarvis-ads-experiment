...
# ---------------------------------------------------------------------------
# Subagent root — all dependencies must be inside jarvis-eda-subagent-net2ads
# ---------------------------------------------------------------------------
_THIS_DIR = Path(__file__).resolve().parent   # translator/
_SUBAGENT = _THIS_DIR.parent                  # jarvis-eda-subagent-net2ads/

# gate_bias_network is vendored into translator/ so the subagent is self-contained.
# Do NOT import from ../jarvis-eda-learning — that path will not exist when deployed.
from translator.gate_bias_network import FETParams, BiasSpecs, calculate_bias
...
_DEFAULT_BIAS_RULES = _SUBAGENT / "ads_pdk" / "bias_rules" / "switch_gate_bias.yaml"
...
def _write_fetbias_netlist(net_path: Path, mappings: List[FetMapping]) -> Path:
    ...
    Topology: internal V_DC source — no VCTRL port exposed to parent.
    The gate control voltage (Vgate) is a design variable overridden per
    instance in the parent SPDT schematic. Rs and Cp are also design
    variables; default values use series FET sizing.

    Topology:
      VGATE (V_DC internal, positive = N_VDD)
        |
      N_VDD --[RS]-- GATE
        |
       [CP]
    ...
    Design variables (all overridable per instance):
      Vgate  — gate control voltage (on=0V, off=-1.5V for PP10)
      Rs     — series gate resistor
      Cp     — RF bypass capacitor
    """
    ...
    if series:
        ...
        rs_shunt   = rs_default
        cp_shunt   = cp_default
    # Cell default Vgate = on-state; each instance overrides to its own Vgate
    vgate_default = VGATE_ON
    ...
    lines = [
        ...
        f".VAR Vgate {vgate_default:.1f} V",
